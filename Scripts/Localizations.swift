...
class Item {
    ...
    init(key : Key, values: [String], arguments: [String]) {
        self.key = key
        self.values = values
...
extension Item {
    ...
    var code : String {
        if arguments.count > 0 {
            ...
            return "static func \(variableName(key.name))(\(args)) -> LocalizeKey { return LocalizeKey(keyPath: \"\(key.original)\", args: [\(formatParams)]) }"
            
        } else {
            return "static func \(variableName(key.name))() -> LocalizeKey { return LocalizeKey(keyPath: \"\(key.original)\") }"
        }
    }
    
    var text: String {
        return "static var \(variableName(key.name))String: String { get { return \"\(key.original)\".localized() } }"
    }
}
...
/// Takes a string key of the form A.B.C##count and returns a parsed Key
/// For example, FOO.BAR.BAZ##other would return (Key(path:[FOO, BAR], name:BAZ))
func parseKey(_ key : String) -> Key {
    ...
    return Key(path:path, name: base, original:key)
}
...
for pair in pairs {
    ...
    do {
        ...
        if let existing = items[key] {
            ...
            let item = Item(key: key, values: [value], arguments: arguments)
            items[key] = item
        }
    }
    catch {
...
var topLevel = Group(name:"Localize_Strings")
...
func printArray(_ group : Group, indent: String) {
    ...
    for item in sortedItems {
        values.append("\"\(item.key.original)\"")
    }
...
func printGroup(_ group : Group, depth : UInt = 0) {
    ...
    print("\n\(indent)struct \(className(group.name)) {", to: &output)
    ...
    if group.items.count > 0 {
        ...
        for item in (group.items.sorted {$0.key.name < $1.key.name}) {
            print("\(childIndent)\(item.text)", to: &output)
